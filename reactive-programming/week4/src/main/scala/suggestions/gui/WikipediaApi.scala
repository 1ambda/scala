...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(x => Success(x)).onErrorReturn(t => Failure(t))
    ...
    def timedOut(totalSec: Long): Observable[T] = obs.take(totalSec seconds)
      /* {
        // ConnectableObservable version
        val o = obs.publish; o.connect
        o.replay(totalSec seconds).refCount.take(totalSec seconds)
      } */
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] =
      ...
      // obs.map(x => requestMethod(x).recovered).flatten
