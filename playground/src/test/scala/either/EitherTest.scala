...
class EitherTest extends FlatSpec with Matchers {
  ...
  "Getcontent" should "return error string for URLs containing 'google'" in {
    getContent(new URL("http://google.com")) match {
      case Left(msg) => println(msg)
      case Right(body) =>
        body.getLines.foreach(println)
        assert(false)
    }
  }

  // Either is designed to be unbiased.
  // So, you can't use an `Either` instance like `Try`, `Option`
  // (`Try` is success-biased)

  // Calling `left` or `right` will return `LeftProjection` or `RightProjection`
