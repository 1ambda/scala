...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateSector(sectorMatrix: SectorMatrix, body: Body): SectorMatrix = {
    sectorMatrix += body
  }

  def mergeSector(s1: SectorMatrix, s2: SectorMatrix): SectorMatrix = {
    s1.combine(s2)
  }
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))(updateSector, mergeSector)
  }
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(Seq[Body]())((acc, b) => {
      acc :+ b.updated(quad)
    }, (acc1, acc2) => {
      acc1 ++ acc2
    })
