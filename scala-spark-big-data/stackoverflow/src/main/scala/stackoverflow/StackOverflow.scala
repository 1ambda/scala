...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.map(kv => {
      val posting = kv._2.head._1
      val scores = kv._2.map(_._2).toArray
      (posting, answerHighScore(scores))
    })
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    val langIndexd = scored
      .map(kv => {
        val optLangIndex = firstLangInTag(kv._1.tags, langs).map(x => (x * langSpread))
        (optLangIndex, kv._2)
      })

    val filtered = langIndexd
      .filter(kv => kv._1.nonEmpty)
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val collected = avgp.collect()

    for(x: (Int, (Int, Int)) <- collected) {
      newMeans(x._1) = x._2
    }
  ...
  //
  //
  def getMedian(xs: Iterable[Int]): Int = {
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    // p: (LangDist, highestScoredInAnswers)
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val medianScore: Int = getMedian(vs.map(_._2))
