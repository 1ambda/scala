...
class BlurSuite extends FunSuite {
  ...
  test("HorizontalBoxBlur.blur with radius 1 should correctly blur the entire 3x3 image (2)") {
    // ref - https://www.coursera.org/learn/parprog1/programming/NeGTv/parallel-box-blur-filter/discussions/threads/GWJHfSKDEea3JA6-1_N9Kw
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
