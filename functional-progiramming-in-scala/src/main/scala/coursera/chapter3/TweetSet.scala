...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // traverse all elems + new
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val acc1 = if (p(elem)) acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, acc1))
  }

  // require traversing all elems
  // since tweets are indexed using tweet bodies, not retweets

  def mostRetweeted: Tweet = {
    var maxTw = elem
    // since Empty throws exception,
    // we need to iterate Tweets using foreach
    foreach( t => if (t.retweets > maxTw.retweets) maxTw = t)
    maxTw
  }

  // If yo have `mostRetweeted`,
  // descendingByRetweet could be one-liner
  def descendingByRetweet: TweetList =
